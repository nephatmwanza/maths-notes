...
\item[(ii).] the \textbf{period} is $\dfrac{360^\circ}{b}$, the horizontal length of one
complete cycle.
\end{enumerate}
\end{definition}
...
Negative index & $a^{-m}=\dfrac{1}{a^m}$ & $2^{-2}=\dfrac{1}{4}$\\
...
(c) $\displaystyle\lim_{x\to 2}\frac{x^2-x-2}{x-2}$.
\end{example}
...
\textbf{(c)} Substituting gives $\frac{4-2-2}{2-2}=\frac{0}{0}$, which is meaningless.
But $\frac{0}{0}$ arising means the numerator and denominator share a factor of
$(x-2)$. Factorise:
$$\frac{x^2-x-2}{x-2}=\frac{(x-2)(x+1)}{x-2}=x+1\qquad\text{for } x\neq 2.$$
...
\begin{example}
Differentiate (a) $y=\dfrac{2x+1}{2x-1}$, \quad (b) $y=\sin^3(2x+5)$, \quad
(c) $y=x^2e^{-2x}$.
\end{example}

\begin{solution}
\textbf{(a) Quotient rule.} Take $u=2x+1$ and $v=2x-1$, so $u'=2$ and $v'=2$:
$$\frac{dy}{dx}=\frac{u'v-uv'}{v^2}=\frac{2(2x-1)-(2x+1)(2)}{(2x-1)^2}
=\frac{4x-2-4x-2}{(2x-1)^2}=\frac{-4}{(2x-1)^2}.$$
The $x$ terms cancel, leaving a derivative that is negative everywhere --- the curve
is always decreasing, which is worth noticing as a check.

\textbf{(b) Chain rule, twice.} Write $y=\left[\sin(2x+5)\right]^3$. The outermost
operation is cubing, then the sine, then the linear inside.
$$\frac{dy}{dx}=3\left[\sin(2x+5)\right]^2\cdot\cos(2x+5)\cdot 2
=6\sin^2(2x+5)\cos(2x+5).$$
Work from the outside in, and multiply by the derivative of the inside at each stage.
The factor of $2$ at the end comes from differentiating $2x+5$, and forgetting it is the
usual chain-rule error.

\textbf{(c) Product rule with a chain inside.} Take $u=x^2$ and $v=e^{-2x}$, so
$u'=2x$ and, by the chain rule, $v'=-2e^{-2x}$:
$$\frac{dy}{dx}=u'v+uv'=2xe^{-2x}+x^2\left(-2e^{-2x}\right)
=2xe^{-2x}-2x^2e^{-2x}.$$
Factorising makes it tidier and easier to use later:
$$\frac{dy}{dx}=2xe^{-2x}\left(1-x\right).$$
\end{solution}
...
\begin{example}
Find $\dfrac{dy}{dx}$ given $2x+3y^2+3x^2y+12=0$.
\end{example}
...
\frac{d}{dx}\left(3x^2y\right) &= 6xy+3x^2\frac{dy}{dx} &&\text{(product rule)}\\
\frac{d}{dx}(12) &= 0
\end{align*}
...
$$\frac{dy}{dx}\left(6y+3x^2\right)=-2-6xy$$
$$\implies\quad \frac{dy}{dx}=\frac{-(2+6xy)}{3x^2+6y}.$$

\begin{note}
The answer contains both $x$ and $y$, and that is normal for implicit differentiation ---
it is not an unfinished answer. The two places to be careful are the $3x^2y$ term, which
...
\begin{example}
Find the gradient of $y=x^3-4x$ at the point where $x=2$, and hence the equation of the
tangent there.
\end{example}

\begin{solution}
$$\frac{dy}{dx}=3x^2-4\quad\implies\quad \left.\frac{dy}{dx}\right|_{x=2}=3(4)-4=8.$$
The gradient is $8$. At $x=2$ the curve is at $y=8-8=0$, so the tangent passes through
$(2,0)$:
