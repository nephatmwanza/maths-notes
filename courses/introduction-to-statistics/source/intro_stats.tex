...
\item and above all making decisions based on the data\\
\end{enumerate}



$*$ Data is information\\\\
There are four types f Data that is
\begin{enumerate}
\item \textbf{Norminal Data}:\\
are used for labeling variables without any quantitative value.\\ Notice that all these labels are mutually exclusive and none of then have numerical significance.
\begin{enumerate}
       \item[$\longrightarrow$] What is your ?
       \item[$\longrightarrow$] Where do you live?
\end{enumerate}
\item \textbf{Ordinal Data}\\
with ordinal data, it is the order of the values is what is important and significant, but the difference between each one is not really known.
\begin{enumerate}
   \item[$\longrightarrow$] How do you feel today?
\end{enumerate}
\item \textbf{Interval Data}\\
These are numerical data in which we know not only the order but also the exact difference between the values.
\item \textbf{Ratio Data}\\
They tell us about the order, or they tell us the exact value between units.  There are two types of quantitative data that we are going to study:
\begin{enumerate}
     \item Discrete data
...
\draw(2.25,4.2)node[above]{Cummulative frequency curve}
...
\subsubsection{Histogram With Unequal Class Intervals}
...
Our interest in this course is quantitative data, we consider whether this is primary data or secondary data.\\


$*$ Primary data is the data that is collected from the source and the methods of collecting data are:
\begin{enumerate}
\item Questionnaires
\item Oral or personal interview (should be guided)
\item Observation
\item Experimental\\
\end{enumerate}
...
\subsubsection{Measures Of Central Tendencies}
...
\textbf{Example}\\
Find the mean of the following values\hspace{0.2cm} 0, 5, 2, 3, 5
$$\overline{X}=\frac{1}{n}\sum^5_1X=\frac{0+5+2+3+5}{5}\hspace{0.5cm}\implies\hspace{0.5cm}\overline{X}=3$$\\
...
\hline
\end{tabular}
$\hspace{1cm}\implies\hspace{0.5cm}\sum f=8$\\
\end{table}


$$\overline{X}=\frac{\sum f(X)}{\sum f}=\frac{9}{8}$$\\
...
\centering
\begin{tabular}{|c|c|c|c|}
\hline C.I & $f$ & Middle point $X$ & $f(X)$\\
\hline $1-5$& 1 & 3 &3\\
$6-10$ & 3 & 8& 24\\
$11-15$ & 2 & 13 & 26\\
$16-20$ & 1 & 18 & 18\\
...
$\displaystyle{\overline{X} =\frac{1}{n}\sum_1^nX=\frac{15+20+21+20+36+15+25+15}{8}}$\\
...
\hline $X$ & $f$ & $f(X)$\\
\hline 0&3&0\\
1&5&5\\ 2&3&6\\ 3&2&6\\ 4&1&4\\ 5&1&5\\
\hline
\end{tabular}
\end{table}

\begin{align*}
\overline{X} &=\frac{\sum f(X)}{\sum f}=\frac{0+5+6+6+4+5}{3+5+3+2+1+1}=\frac{26}{15}\\\\ \implies\hspace{0.5cm} \overline{X}&\approx 1.733\\
\end{align*}
...
The first consideration is the type of data. If the variable is categorical, the mode is the single measure that best describe the data.\\ The second consideration in selecting the index is to ask whether the total of all observation is of any interest, if the answer is yes, then the mean is the proper index of the central tendency. If the total is of no interest then depending on whether the histogram is symmetric on skewed one must use either mean or median respectively.\\ In all cases the histogram must be unmodel.\\

\subsubsection{Measure Of Dispersion}
...
$Q_3$ is the $\dfrac{3}{4}\times 11^{\text{th}}$ observation $\dfrac{3}{4}\times 11=\frac{33}{4}=8.2\implies Q_3$ lies between $8^{\text{th}}$ value and $9^{\text{th}}$ value.\\\\
...
\hline C-I & $f$& $X$ & $f D_X$ & $f D_X^2$ & $U=\frac{X-a}{h}$& $f U_X^2$\\
&&&$X-a$&&$f U_X$ &\\
\hline
$0-10$&5&5&&&&\\
$10-20$&10&15&&&&\\
...
&=\frac{1}{n}\Bigg(\sum X^2-2\overline{X}\sum X +\sum \overline{X}^2\Bigg)=\frac{1}{n}\Bigg[\sum X^2-2n\overline{X}^2+n\overline{X}^2\Bigg]\\\\
...
\subsubsection{Properties of $E(X)$}
\begin{enumerate}
...
\subsubsection{Properties of Covariance (Cov$(X,Y)$)}
\begin{enumerate}
...
(3.2,0)--(3.2,0.91)
(2.1,0)--(2.1,1.8);
\draw(6.5,1)node[right]{$+ve$ Skewness}
(1,0)node[below]{Mode}
...
(-3.2,0)--(-3.2,0.91)
(-2.1,0)--(-2.1,1.8);
\draw(1,1)node[right]{$-ve$ Skewness}
(-1,0)node[below]{Mode}
(-3.2,0)node[below]{Mean};
\end{tikzpicture}
\end{figure}
...
\item If $SK>0$, then the distribution is $+vely$ skewed.
\item If $SK<0$, then the distribution is $-vely$ skewed.\\
\end{enumerate}
...
Probability distribution for a $+ vely$ biased estimator.\\
...
\textbf{Definition}\\
The biase $B$ of an estimator $\widehat{\theta}$ is given by $B=E(\hat{\theta})-\theta$.\\ 
...
If the sampling is without replacement and population size is $N$ then the sampling distribution of the mean becomes $$\text{var}(\overline{X})=\frac{\sigma^2}{n}\Bigg(\frac{N-n}{N-n}\Bigg)....................(2)$$
If $n$ is very much smaller than $N$ then (2) gives approximately the same result as (1).\\ Whether the sampling is with or without replacement var$(\overline{X})$ decreases as the sample size increases.\\ That means the lager the sample size the closer the sample mean is likely to the population mean
$$E(S^2)=\frac{\sum (X-\mu)^2}{n}\hspace{1cm}\text{unbiased}$$\\ $$E(S^2)=\frac{\sum (X-\overline{X})^2}{n}\hspace{1cm}\text{biased}$$\\
...
\end{align*}
This gives us unbiased estimator of variance, it is denoted by $\widehat{S}^2$ $$\therefore\hspace{0.3cm} \widehat{S}^2=\frac{\sum (X-\overline{X})^2}{n-1}.............(2)$$ and $\widehat{S}^2$ and $S$ are related by \hspace{0.2cm}$\displaystyle{\widehat{S}^2=\frac{n}{n-1}S^2,\hspace{1cm} \sigma^2=\widehat{S}^2}$\\
...
\end{center}
If the probability that a member of a population posses an attributes $P$, then we know that $X$ the number of members of sample size $n$ which posses this attribute is Binomialy distributed $B(n,P)$, we expect $P_s=X/n$.\\ The proportion in the sample which shows the attribute to be an unbiased estimator of $P$ and we can show it as;
\begin{align*}
...
\item Five measurements of the volume of acid required in a tiltration are; 25.1, 25.2, 25.2, 25.0, 25.5 cm$^3$.\\ Use these results to obtain estimation for the mean and s.d of the volume required.\\
\end{enumerate} 
...
\path[pattern=north west lines, smooth, domain=-3.5:-1.5, variable=\x]plot(\x,{6*1/sqrt(2*pi)*exp(-1/2*\x*\x)})--(-1.5,0.75)--(-1.5,0)--(-3.5,0);
\draw(-1.5,0)--(-1.5,0.77)
(1.5,0)--(1.5,0.75)
(-3,-0.5)--(-2,0.1);
...
\begin{enumerate}
\item For large sample size ($\sigma=$ known) our class interval$$\text{C.I is:}\hspace{0.5cm} \overline{X}\pm Z_{\alpha/2}\times \frac{\sigma}{\sqrt{n}}$$
\item For large sample size ($\sigma=$ unknown) $$\text{C.I is}:\hspace{0.5cm} \overline{X}\pm Z_{\alpha/2}\times \frac{S}{\sqrt{n-1}}\hspace{0.5cm}\text{or}\hspace{0.5cm} \overline{X}\pm Z_{\alpha/2}\times \frac{\widehat{S}}{\sqrt{n}}$$
\item C.I for small sample size ($\sigma=$ known) $$\overline{X}\pm t_{n-1,\alpha/2}\times \frac{\sigma}{\sqrt{n}}$$
\item C.I for small sample size ($\sigma=$ unknown) $$\overline{X}\pm t_{n-1,\alpha/2}\times \frac{S}{\sqrt{n-1}}\hspace{0.5cm}\text{or}\hspace{0.5cm} \overline{X}\pm t_{n-1,\alpha/2}\times \frac{\widehat{S}}{\sqrt{n}}$$
\item C.I of a proportion from large sample size $$P\pm Z_{\alpha/2}\times \sqrt{\frac{P(1-P)}{n}}$$
\item C.I of a proportion for  small sample size $$\widehat{P}\pm t_{n-1,\alpha/2}\times \sqrt{\frac{\widehat{P}(1-\widehat{P})}{n}}$$
\item C.I for the difference between two means: $$(\overline{X}_1-\overline{X}_2)\pm Z_{\alpha/2}\times \sqrt{\frac{\sigma_1^2}{n_1}+\frac{\sigma_2^2}{n_2}}$$
\item $$(\widehat{P}_1-\widehat{P}_2)\pm Z_{\alpha/2}\times \sqrt{\frac{\widehat{P}_1(1-\widehat{P}_1)}{n_1}+\frac{\widehat{P}_2(1-\widehat{P}_2)}{n_2}}$$
\item If you draw two samples from the same population, then $$(\overline{X}_1-\overline{X}_2)\pm Z_{\alpha/2}\times \text{SP}\sqrt{\frac{1}{n_1}+\frac{1}{n_2}}$$ where SP is called the Pooled variance $$\text{SP}=\sqrt{\frac{(n_1-1)S^2_1+(n_2-1)S^2_2}{n_1+n_2-2}}\hspace{0.5cm}\text{and}\hspace{0.5cm}0<\frac{S^2_1}{S^2_2}<2$$\\
\end{enumerate}
...
(2,1.5)node[right]{Critica Region};
...
\textit{Interpretation}: If the statistic $Z$ lie in the C.R this means we reject $H_0$ and accept $H_1$. That means $H_1$ is true.\\
If the statistic $Z$ lies in the acceptance region that means we accept $H_0$ and reject $H_1$. That means $H_0$ is true.\\
...
\begin{enumerate}
\item If the P-value is smaller than the level of significance $\alpha/2=0.025$. That means we reject $H_0$ and Accept $H_1$.
\item If the P-value is grater than the level of significance we Accept $H_0$ and reject $H_1$.\\
\end{enumerate}
...
\draw(-3.5,0)--(3.5,0);
\draw[smooth,domain=-3:3,variable=\x]plot(\x,{6*1/sqrt(2*pi)*exp(-1/2*\x*\x)});
\path[pattern=north west lines, smooth, domain=1.5:3, variable=\x]plot(\x,{6*1/sqrt(2*pi)*exp(-1/2*\x*\x)})--(3,0)--(1.5,0)--(1.5,0.75);
\path[pattern=north west lines, smooth, domain=-3:-1.5, variable=\x]plot(\x,{6*1/sqrt(2*pi)*exp(-1/2*\x*\x)})--(-1.5,0.75)--(-1.5,0)--(-3,0);
\draw(-1.5,0)--(-1.5,0.77)
(1.5,0)--(1.5,0.75);
...
(3,-0.5)node[right]{Criticla Region};
\end{tikzpicture}
\end{figure}
...
Do the number of individuals or objects that fall in each category differ significantly from the number you would expect? Is there a difference between sampling error or is it test.\\\\
\textbf{$\chi^2$ Test}
\begin{enumerate}
...
\item The expected class frequencies are not less 5.\\
\end{enumerate}

If the second condition is not met it can be improved where $v=1$ by making an adjacement known as YATES correction which involves reducing value of $|O-e|$ by 1/2.\\\\
...
\draw[-Stealth](6.5,0.8)--(6,0.8)--(6,0.2);
\draw[-Stealth](5.5,-0.6)--(5,-0.6)--(5,0);
\draw(6.5,0.8)node[right]{Criticla Region}
...
\hline $C_1$& $C_2$ &$g$
\end{tabular}
$\hspace{0.5cm}$ This is a $2\times 2$ contingent table df $=(r-1)(c-1)$\\
\end{table}
...
\vspace{1cm}
The $\chi^2$ distribution depends on the df, therefore we write $\chi^2_v$. That is $v=n-1$ is called degree of freedom is the number of expected frequencies which can be varied independently $2\times 2$ contingency table has $v=1$ df because a column losses 1 df and the rows lose 1 df to give $v=(2-1)(2-1)$. The table below shows the seven results for a random sample of 4146 people on the type of house and compound.
...
SS_B = \text{Between samples sum of squares} & = SS_B = \sum_{j=1}\frac{T^2}{n_j}-\frac{T^2}{n}\\\
SS_W = \text{Within sample sum of squares} &= SS_W =SS_T - SS_B\\\\
MS_e & =\text{sum of standard errors}\\
\end{align*}
...
&&&&\\
Within Sample & $SS_{\varepsilon}$ & $v_2=n-k$ & $\dfrac{SS_{\varepsilon}}{n-1}$ &\\
\hline
Total & $SS_T$ & $n-1$ &&\\
\hline
\end{tabular}
...
$$\sum_{i=1}^k \sum_{j=1}^n(x_{ij}-\overline{x}^2)=\sum^k_{i=1}(\overline{x}_i-\overline{x})^2+\sum_{i=1}^k\sum^n_{j}(x_{ij}-\overline{x})^2$$
...
(2.5,2.5)node[]{$\ast$}
(3,3)node[]{$\ast$};
\draw(5,2)node[right]{Perfect $+ve$ correlation}
(-2,2)node[left]{1};
\end{tikzpicture}
\end{figure}
...
(-1.5,0)--(-1.5,0.5)
(1.5,0)--(1.5,0.5);
\path[pattern=north east lines](1.5,0)--(1.5,0.5)--(-1.5,0.5)--(-1.5,0)--cycle;
...
(-1.5,-0.1)node[below]{-0.4}
(3.8,-1.2)node[right]{Perfect $+ve$ correlation}
(-3.8,-1.2)node[left]{Perfect $-ve$ correlation}
(0,-1.1)node[below]{No linear correlation}
(3.6,1)node[right]{$+ve$ fair correlation}
(-3.6,1)node[left]{$-ve$ fair correlation};
...
\pagebreak
\subsubsection{Spearmans Rank Correlation Coefficient}
...
\end{align*}
There is a $+ve$ fair correlation between the judges. So we have that $r_s=0.6875$\\

Now we solve for the second part.\\
Set the hypothesis\\
$H_0$: The  is no correlation between the judges $r_s=0$\\ $H_1:$ There is an association $r_s\neq 0$\\\\ $|r_c|=$ critical value from the table, $n=7$, $|r_c|=0.75$\\ $$r_s=0.6875$$ We Accept $H_0$ since $r_s<|r_c|=0.75$.\\  Therefore, the judges are not in agreement.\\\\
...
$$Y_i=\alpha +\beta X_i+e_i,\hspace{1cm}\text{and}\hspace{1cm} e_i=Y_1-\alpha -\beta X_i$$
...
The parameters $\alpha$ and $\beta$ are estimated by the method of least squares so called because estimates $\widehat{a}$ and $\widehat{b}$ of $\alpha$ and $\beta$ respectively are chosen so as to minimize $\sum\limits_i^ne_i's$ ($n$ is the number of points on the scatter diagram).
\begin{align*}
Y_i &=\alpha +\beta X_i+e_i\\e_i&=Y_i-\alpha -\beta X_i\\ \sum^n_i e^2_is &=\sum^n_{i=1}(Y_i-\alpha -\beta X_i)^2
\end{align*}
...
\begin{align*}
\frac{\partial}{\partial \widehat{a}}\Bigg(\sum^n_i(Y-\widehat{a}-\widehat{b}X)^2\Bigg)&=0\\\\ 2\Bigg(\sum^n_i(Y-\widehat{a}-\widehat{b}X)\Bigg)(-1) &=0\\\\ -2\sum^n_i(Y-\widehat{a}-\widehat{b}X)&=0.......................(1)\\
\end{align*}

\begin{align*}
\frac{\partial}{\partial \widehat{b}}\Bigg(\sum^n_i(Y-\widehat{a}-\widehat{b}X)^2\Bigg)&=0\\\\ -2\sum^n_iX(Y-\widehat{a}-\widehat{b}X)&=0.........................(2)\\
\end{align*}

\begin{align*}
\text{Now using equation (1) we have the following}\\
\sum^n_i(Y-\widehat{a}-\widehat{b}X)&=0\\\\ \sum^n_iY-n\widehat{a}-\widehat{b}\sum^n_iX&=0......................(3)\\\\ n\widehat{a}=\sum^n_iY-\widehat{b}\sum^n_iX\hspace{0.5cm}&\implies\hspace{0.5cm} \widehat{a}=\frac{1}{n}\Bigg[\sum^n_iY-\widehat{b}\sum^n_iX\Bigg]\\\\\\ \implies\hspace{0.5cm}\widehat{a}&=\overline{Y}-\widehat{b}\overline{X}\\\\
\end{align*}

\begin{align*}
\text{Also from equation (3), it follows that}\\
...
Therefore, the relationship between $X$ and $Y$ is given by \hspace{0.1cm}$Y=\widehat{a}+\widehat{b}X\hspace{0.1cm}$ and this is called the Regression line of $Y$ on $X$.\\\\ The regression line will pass through the point $(\overline{X},\overline{Y})$.
...
$$t_{8,0.025}=2.31\hspace{1cm}\text{and}\hspace{1cm} \hat{b}=6.08$$\\
\begin{align*}
