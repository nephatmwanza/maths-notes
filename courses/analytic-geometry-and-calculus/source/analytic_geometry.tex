...
Conic sections, as the name implies are curve obtained by cutting a cone with a plane. Depending on the inclination of the cutting plane to the axis of the cone, the following curves occur
...
$$\implies\hspace{0.5cm} a^2 (x-c)^2 + y^2 = a^4 -a^2cx + c^2x^2$$

$$\implies\hspace{0.5cm} a^2x^2 - 2a^2cx + a^2c^2 + a^2y^2 = a^4 -a^2cx + c^2x^2$$
...
\item\textbf{limit of a sum of functions}
...
\item \textbf{limit of a product of functions}
...
\item \textbf{limit of a quotient of functions}
...
\textbf{NOTE:} that at every stage of trying to find the limit we are getting the indeterminate form $\Big(\dfrac{\infty}{\infty}\Big)$. Thus, we may need to rewrite the limit as 
\begin{align*}
...
\textbf{Other examples of indeterminate forms}
...
\textbf{Note:} $f(x) = (x - 1) (x - 2) = x^2 - 3x + 2$
...
\textbf{Some Properties of Sequences}
\begin{enumerate}
...
\item \textbf{The Comparison test}
...
\pagebreak
\textbf{Convergence for alternating series}
...
\item \textbf{Ratio Test for Absolutely Convergent series}
...
\textbf{Interval of Convergence of a Power series}
...
If $f(x)$ is an antiderivative of $F(x)$, then $f(x) + c$ is an antiderivative of for every value of the constant $c$ for it $\dfrac{dF}{dx} = f$, then 
\begin{align*}
...
The set of all antiderivatives of a function $f(x)$ is called the indefinite of $f$ with respect to $x$. The notation for the indefinite integral is $\displaystyle{\int f(x)dx}$. When a formula $F(x) + c$ gives all the antiderivatives of $f$, we indicate this by writing $$\int f(x) dx = F(x) + c$$
...
\textbf{Note:} If $\hspace{0.2cm} u = e^{2x}\hspace{0.2cm}$ and $\hspace{0.2cm} dv = x^2\hspace{0.1cm}dx$, then $\hspace{0.2cm}du = \dfrac{1}{2}e^{2x}\hspace{0.2cm}$ and $\hspace{0.2cm} v = \dfrac{1}{3}x^3$
...
\begin{enumerate}
\item For $\displaystyle{\int \sin^mx\cos^nxdx}\hspace{0.2cm}$: 
\begin{enumerate}
...
\item For $\hspace{0.3cm}\displaystyle{\int \tan^m x \sec^nx\hspace{0.2cm}dx}$
\begin{enumerate}
...
For any zero vector $A$, we obtain a unit vector called the direction of $A$ by dividing $A$ by its own length, i.e $$\text{Direction of }\hspace{0.3cm} A = \frac{A}{
\left|A\right|
}$$
...
\textbf{Work and the Dot Product}
...
\textbf{The Dot Product in Component form}
...
\textbf{Properties of Dot Product}
...
\textbf{NOTE:} the vector $a\hat{i} + b\hat{j} + c\hat{k}$ is parallel to the straight line passing through the points $P$ and $Q$.
...
\textbf{Note} that is if $\alpha = -(\underline{a}\times \underline{b}) = (-\underline{a})\times \underline{b}=\underline{a}\times (-\underline{b}).$
...
(Write a complete proof showing all the stages)
$$\underline{a} \times \underline{b} = (a_1\textbf{i} + a_2 \textbf{j} + a_3\textbf{k})\times  (b_1\textbf{i} + b_2 \textbf{j} + b_3\textbf{k})                            =
\begin{vmatrix}
\textbf{i} & \textbf{j} & \textbf{k}\\
a_1 & a_2 & a_3\\
b_1 & b_2 & b_3\\
...
\begin{align*}
h & = \begin{vmatrix}
\left|\underline{a}\right|\cos \theta
\end{vmatrix}\hspace{0.2cm}\text{Area of base}\hspace{0.2cm} = \left|\underline{b}\times \underline{c}\right|\\\\
V & = \left|\underline{a}\right|\left|\cos \theta\right|\left|\underline{b}\times \underline{c}\right|\\
& = \begin{vmatrix}
\left|\underline{a}\right|\left|\underline{b}\times \underline{c}\right|\cos\\
\end{vmatrix}\\
...
\textbf{NOTE:} Lines in space that do not intersect are not parallel and called skew lines, such lines can not lie in the same plane.
...
\textbf{Functions of Several Variables}
...
\textbf{Note:} $\dfrac{\partial f}{\partial x}$ is the is the ordinary derivative of $f$ with respect to $x$ with $y$ held fixed.
...
\textbf{The Error in the Standard Linear Approximation of $f(x,y)$ Near $(a,b)$}
...
and having the property that $$\frac{\partial M}{\partial y}= \frac{\partial N}{\partial x} \hspace{0.3cm}\cdots\cdots\cdots\hspace{0.4cm} (9)$$ is said to be exact. To solve such an equation we find a function $f(x,y)$ such that $$df = M(x,y)\hspace{0.1cm}dx + N(x,y)\hspace{0.1cm}dy \hspace{0.3cm} \cdots\cdots\cdots\hspace{0.3cm} (10)$$ so that (8) becomes $df = 0$ and the solution $f(x,y)$ satisfies $f(x,y) = C$, $C$  a constant and $f$ satisfies $\dfrac{\partial f}{\partial x} = M(x,y)$ and $\dfrac{\partial f}{\partial y} = N(x,y)$.
...
\pagebreak
\textbf{MAT 2100- ANALYTIC GEOMETRY AND THE CALCULUS}


\textbf{Pre-requisite: MAT 1100 Foundation Mathematics}
